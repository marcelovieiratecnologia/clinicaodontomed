...
def listar_entrada_saida(request):
		...
		return render(request, 'caixa/listar_entrada_saida.html', {"imprimir_variavel": imprimir_variavel})

def cadastrar_entrada_saida(request):
		if request == "POST": # vejo se esta sendo um POST
				...
				if form_entrada_saida.is_valid(): # se for válido sigo 
						...
						# Crio uma nova variavel que ai receber os dados , para que eu passe para o meu service/entrada_saida_service.py 
